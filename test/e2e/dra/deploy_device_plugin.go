...
// deployDevicePlugin deploys https://github.com/kubernetes/kubernetes/tree/v1.35.0/test/images/sample-device-plugin
// on the chosen nodes.
//
...
func deployDevicePlugin(tCtx ktesting.TContext, f *framework.Framework, nodeNames []string, skipCleanup bool) v1.ResourceName {
	...
	for _, nodeName := range nodeNames {
		gomega.Eventually(tCtx, func() int64 {
			...
			// We expect 2 devices to be allocatable on each node because that's what the sample plugin reports:
			// https://github.com/kubernetes/kubernetes/blob/111a2a0d2dfe13639724506f674bc4f342ccfbab/test/images/sample-device-plugin/sampledeviceplugin.go#L102-L105
			return e2enode.CountSampleDeviceAllocatable(node)
		}).WithTimeout(f.Timeouts.PodStart).Should(gomega.BeNumerically("==", 2), "expected 2 %q to be allocatable on node %q", e2enode.SampleDeviceResourceName, nodeName)
	}
