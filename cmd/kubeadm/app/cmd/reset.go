...
func resetWithCrictl(execer utilsexec.Interface, dockerCheck preflight.Checker, criSocketPath, crictlPath string) {
	if criSocketPath != "" {
		...
		if err != nil {
			fmt.Printf("[reset] failed to list running pods using crictl: %s. Trying using docker instead\n", err)
			resetWithDocker(execer, dockerCheck)
			return
		}
		...
		for _, s := range sandboxes {
			...
			if err := execer.Command(crictlPath, params...).Run(); err != nil {
				fmt.Printf("[reset] failed to stop the running containers using crictl: %s. Trying using docker instead\n", err)
				resetWithDocker(execer, dockerCheck)
				return
			}
			...
			fmt.Printf("[reset] executing command %s %s\n", crictlPath, strings.Join(params, " "))
			if err := execer.Command(crictlPath, params...).Run(); err != nil {
				fmt.Printf("[reset] failed to remove the running containers using crictl: %s. Trying using docker instead\n", err)
				resetWithDocker(execer, dockerCheck)
				return
			}
		}
