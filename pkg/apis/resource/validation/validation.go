...
func validateAttributeName(name string, fldPath *field.Path) field.ErrorList {
	...
	switch len(parts) {
	case 1:
		allErrs = append(allErrs, validateAttributeIdentifier(parts[0], fldPath.Child("identifier"))...)
	case 2:
		allErrs = append(allErrs, validateDriverName(parts[0], fldPath.Child("domain"))...)
		allErrs = append(allErrs, validateAttributeIdentifier(parts[1], fldPath.Child("identifier"))...)

	}
	return allErrs
}

func validateAttributeIdentifier(id string, fldPath *field.Path) field.ErrorList {
	allErrs := corevalidation.ValidateDNS1123Label(id, fldPath)
	if len(id) > resource.DeviceAttributeMaxIDLength {
		allErrs = append(allErrs, field.TypeInvalid(fldPath, id, fmt.Sprintf("must not be longer than %d characters", resource.DeviceAttributeMaxIDLength)))
	}
	return allErrs
}
