...
func (m *kubeGenericRuntimeManager) doPodResizeAction(pod *v1.Pod, podContainerChanges podActions, result *kubecontainer.PodSyncResult) error {
	...
	if podResources == nil {
		...
		err := fmt.Errorf("unable to get resource configuration processing resize for pod %s", pod.Name)
		result.Fail(err)
		return err
	}
	...
	if err != nil {
		...
		err := fmt.Errorf("unable to get pod cgroup memory config for pod %s", pod.Name)
		result.Fail(err)
		return err
	}
	...
	if err != nil {
		...
		err := fmt.Errorf("unable to get pod cgroup cpu config for pod %s", pod.Name)
		result.Fail(err)
		return err
	}
	...
	if len(podContainerChanges.ContainersToUpdate[v1.ResourceCPU]) > 0 || podContainerChanges.UpdatePodResources {
		if podResources.CPUShares == nil {
			...
			err := fmt.Errorf("podResources.CPUShares is nil for pod %s", pod.Name)
			result.Fail(err)
			return err
		}
...
func (m *kubeGenericRuntimeManager) SyncPod(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, backOff *flowcontrol.Backoff, resizeInProgressUpdater kubecontainer.ResizeInProgressUpdater) (result kubecontainer.PodSyncResult) {
	...
	if resizable, _ := IsInPlacePodVerticalScalingAllowed(pod); resizable {
		if len(podContainerChanges.ContainersToUpdate) > 0 || podContainerChanges.UpdatePodResources {
			err := m.doPodResizeAction(pod, podContainerChanges, &result)
			if err == nil {
				resizeInProgressUpdater.SetPodResizeInProgressCondition(pod.UID, "", "")
			} else {
				resizeInProgressUpdater.SetPodResizeInProgressCondition(pod.UID, v1.PodReasonError, err.Error())
			}
		}
	}
