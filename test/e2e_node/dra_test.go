...
const (
	...
	pluginRegistrationTimeout = time.Second * 30 // how long to wait for a node plugin to be registered
)
...
var _ = ginkgo.Describe("[sig-node] DRA [Feature:DynamicResourceAllocation][NodeFeature:DynamicResourceAllocation]", func() {
	...
	ginkgo.Context("Resource Kubelet Plugin [Serial]", func() {
		...
		ginkgo.It("must register after Kubelet restart", func(ctx context.Context) {
			ginkgo.By("reset registration info for the Kubelet Plugin")
			kubeletPlugin.Reset()
			gomega.Expect(kubeletPlugin.IsRegistered()).Should(gomega.BeFalse(), "Kubelet plugin should not be registered")

			ginkgo.By("restarting Kubelet")
			restartKubelet(true)

			ginkgo.By("wait for Kubelet plugin re-registration")
			gomega.Eventually(kubeletPlugin.IsRegistered).WithTimeout(pluginRegistrationTimeout).Should(gomega.BeTrue(), "Kubelet plugins should be registered")
		})
		...
		ginkgo.It("must register after plugin restart", func(ctx context.Context) {
			...
			ginkgo.By("wait for Kubelet plugin re-registration")
			gomega.Eventually(kubeletPlugin.IsRegistered).WithTimeout(pluginRegistrationTimeout).Should(gomega.BeTrue())
		})
...
func newKubeletPlugin(nodeName string) *testdriver.ExamplePlugin {
	...
	gomega.Eventually(plugin.IsRegistered).WithTimeout(pluginRegistrationTimeout).Should(gomega.BeTrue())
