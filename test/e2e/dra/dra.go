...
var _ = framework.SIGDescribe("node")("DRA", feature.DynamicResourceAllocation, func() {
	...
	driverTest := func(parameterMode parameterMode) {
		...
		maxAllocations := 1
		numPods := 10
		driver := NewDriver(f, nodes, perNode(maxAllocations, nodes)) // All tests get their own driver instance.
		...
		ginkgo.It("supports sharing a claim", func(ctx context.Context) {
			objects := genParameters()
	...
	ginkgo.Context("driver", func() {
		...
		ginkgo.Context("with ConfigMap and structured parameters", func() { driverTest(parameterModeTranslated) })
		ginkgo.Context("with only structured parameters", func() { driverTest(parameterModeStructured) })
	})
...
func (b *builder) class() *resourcev1alpha2.ResourceClass {
	class := &resourcev1alpha2.ResourceClass{
		...
		StructuredParameters: b.driver.parameterMode != parameterModeConfigMap,
	}
				...
				Requests: []resourcev1alpha2.ResourceRequest{
					{
						...
						ResourceRequestModel: resourcev1alpha2.ResourceRequestModel{
							NamedResources: &namedresourcesapi.Request{
								Selector: "true",
							},
						},
					},
