...
type Driver interface {
	...
	GetClassParameters(ctx context.Context, class *corev1.ResourceClass) (interface{}, error)
	...
	//
	// The caller will wrap the error to include the parameter reference.
	GetClaimParameters(ctx context.Context, claim *corev1.ResourceClaim, class *corev1.ResourceClass, classParameters interface{}) (interface{}, error)
	...
	//
	// If selectedNode is set, the driver must attempt to allocate for
	...
	// must be idempotent.
	Allocate(ctx context.Context, claim *corev1.ResourceClaim, claimParameters interface{}, class *corev1.ResourceClass, classParameters interface{}, selectedNode string) (*corev1.AllocationResult, error)
	...
	// the claim.
	StopAllocation(ctx context.Context, claim *corev1.ResourceClaim) error
	...
	// must be idempotent.
	Deallocate(ctx context.Context, claim *corev1.ResourceClaim) error
...
func New(
	...
	logger := klog.FromContext(ctx).WithName("resource controller")
	...
	podSchedulingInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    ctrl.add,
		UpdateFunc: ctrl.update,
		DeleteFunc: ctrl.delete,
	})
...
func (ctrl *controller) add(obj interface{}) {
	ctrl.addNewOrUpdated("Adding new work item", obj)
}

func (ctrl *controller) update(oldObj, newObj interface{}) {
...
func (ctrl *controller) sync() {
	...
	logger := ctrl.logger.WithValues("key", key)
...
// and does it.
func (ctrl *controller) syncClaim(ctx context.Context, claim *corev1.ResourceClaim) error {
	var err error
...
func (ctrl *controller) getParameters(ctx context.Context, claim *corev1.ResourceClaim, class *corev1.ResourceClass) (claimParameters, classParameters interface{}, err error) {
	classParameters, err = ctrl.driver.GetClassParameters(ctx, class)
	if err != nil {
		err = fmt.Errorf("class parameters %s: %v", class.Parameters, err)
		return
	}
	claimParameters, err = ctrl.driver.GetClaimParameters(ctx, claim, class, classParameters)
	if err != nil {
		err = fmt.Errorf("claim parameters %s: %v", claim.Spec.Parameters, err)
		return
	}
	return
}
...
func (ctrl *controller) allocateClaim(ctx context.Context,
	claim *corev1.ResourceClaim, claimParameters interface{},
	class *corev1.ResourceClass, classParameters interface{},
	selectedNode string,
	selectedUser *corev1.ResourceClaimUserReference) error {
	logger := klog.FromContext(ctx)
	claim = claim.DeepCopy()
...
func (ctrl *controller) checkPodClaim(ctx context.Context, pod *corev1.Pod, podClaim corev1.PodResourceClaim) (*ClaimAllocation, error) {
	...
	if claim.Spec.AllocationMode != corev1.AllocationModeWaitForFirstConsumer {
...
// and does it.
func (ctrl *controller) syncPodScheduling(ctx context.Context, podScheduling *corev1.PodScheduling) error {
	logger := klog.FromContext(ctx)
	...
	if selectedNode != "" {
		...
		if unsuitable {
			...
			selectedUser := &corev1.ResourceClaimUserReference{
				Version:  "v1",
				Resource: "pods",
				Name:     pod.Name,
				UID:      pod.UID,
	...
	for _, delayed := range claims {
		i := 0
		for i = range podScheduling.Status.Claims {
			if podScheduling.Status.Claims[i].PodResourceClaimName == delayed.PodClaimName {
				break
			}
		}
		if i == len(podScheduling.Status.Claims) {
		...
		if stringsDiffer(podScheduling.Status.Claims[i].UnsuitableNodes, delayed.UnsuitableNodes) {
			modified = true
			podScheduling.Status.Claims[i].UnsuitableNodes = delayed.UnsuitableNodes
		}
	}
	if modified {
		if _, err := ctrl.kubeClient.CoreV1().PodSchedulings(podScheduling.Namespace).UpdateStatus(ctx, podScheduling, metav1.UpdateOptions{}); err != nil {
...
func (ctrl *controller) hasFinalizer(claim *corev1.ResourceClaim) bool {
